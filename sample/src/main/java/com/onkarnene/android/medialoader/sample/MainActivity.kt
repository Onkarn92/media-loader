...
class MainActivity : AppCompatActivity() {
	...
	override fun onResume() {
		super.onResume()
		MediaLoader.Builder<AppCompatImageView>(this).load(full).into(image).create().download()
		MediaLoader.Builder<AppCompatImageView>(this).load(url2).into(image1).create().download()
		MediaLoader.Builder<AppCompatImageView>(this).load(url3).into(image2).create().download()
		MediaLoader.Builder<AppCompatImageView>(this).load(url4).into(image3).create().download()
	}
}